...
func start_board(_map_grid):
	...
	for x in range(matrix.size()):
		for y in range(matrix[0].size()):
			...
			if _map_grid[x][y] == 1:
				...
				matrix[x][y].set_weight(randi() % 5 + 1)
	...
	$DijkstraPathfinder.setup(matrix, ConnectionsDir)
		...
		if connection_info.End.check_token_amount() == 0:
			...
			for i in $Itens.get_children():
				if !i.get_gotten():
					...
					$DijkstraPathfinder.find_path_2(matrix[desired_position.x][desired_position.y], i.get_place_holding_this())
					pass
